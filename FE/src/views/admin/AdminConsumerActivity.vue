<template>
  <div class="consumer-activity-wrap">
    <div class="crumbs">
      <el-breadcrumb separator="/">
        <el-breadcrumb-item>
          <i class="el-icon-fa fa-edit"></i> 选择管理
        </el-breadcrumb-item>
      </el-breadcrumb>
    </div>

    <div class="container">
      <div class="query-form">
        <el-row :gutter="20">
          <el-col :span="2">
            <el-button @click="create" icon="el-icon-plus">创建</el-button>
          </el-col>
          <el-col :span="2">
            <el-checkbox
              @change="updateAllowSelect"
              v-model="optionForm.allowSelect"
              >允许用户选择
            </el-checkbox>
          </el-col>
          <el-col :offset="13" :span="3">
            <el-input
              @keyup.enter.native="query"
              placeholder="活动名"
              v-model="queryForm.name"
            />
          </el-col>
          <el-col :span="3">
            <el-button @click="query" icon="el-icon-search" type="primary"
              >搜索
            </el-button>
          </el-col>
        </el-row>
      </div>

      <el-row justify="center" type="flex">
        <el-pagination
          :current-page.sync="pageIndex"
          :page-size="pageSize"
          :total="pageSize * pageCount"
          @current-change="getPage"
          background
          layout="prev, pager, next"
        >
        </el-pagination>
      </el-row>

      <div class="table">
        <el-table :data="tableData" stripe>
          <el-table-column align="center" label="选择Id" prop="id" width="80px" />
          <el-table-column align="center" label="活动名" prop="activityName" />
          <el-table-column align="center" label="用户名" prop="consumerName" />
          <el-table-column align="center" label="操作" width="200px">
            <template slot-scope="scope">
              <el-button @click="edit(scope.row.id)" size="mini" type="success"
                >编辑
              </el-button>
              <el-button
                @click="deleteItem(scope.row.id)"
                size="mini"
                type="danger"
                >删除
              </el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>

      <el-dialog :visible.sync="editing" title="编辑" width="30%">
        <el-form :model="entityForm" label-width="70px" ref="form">
          <el-form-item label="用户">
            <el-select
              :disabled="entityForm.id !== -1"
              placeholder="请选择用户"
              v-model="entityForm.consumerId"
            >
              <el-option
                :key="index"
                :label="item.name"
                :value="item.id"
                v-for="(item, index) in consumers"
              />
            </el-select>
          </el-form-item>
          <el-form-item label="活动">
            <el-select
              :disabled="entityForm.id !== -1"
              placeholder="请选择活动"
              v-model="entityForm.activityId"
            >
              <el-option
                :key="index"
                :label="item.name"
                :value="item.id"
                v-for="(item, index) in activitys"
              />
            </el-select>
          </el-form-item>

        </el-form>
        <span class="dialog-footer" slot="footer">
          <el-button @click="save" type="primary">确 定</el-button>
          <el-button @click="editing = false">取 消</el-button>
        </span>
      </el-dialog>
    </div>
  </div>
</template>

<script>
import * as api from "../../api/admin/consumerActivity";
import * as ActivityApi from "../../api/admin/activity";
import * as ConsumerApi from "../../api/admin/consumer";
import * as OptionApi from "../../api/option";

export default {
  name: "AdminConsumerActivity",
  data() {
    return {
      queryForm: {
        className: "",
        activityName: "",
        consumerName: ""
      },
      optionForm: {
        allowSelect: true,
        allowGrade: true
      },
      entityForm: {},
      tableData: [],
      pageSize: api.pageSize,
      pageCount: 1,
      pageIndex: 1,
      editing: false,
      activitys: [],
      consumers: []
    };
  },
  methods: {
    query() {
      api
        .getPageCount(
          this.queryForm.className,
          this.queryForm.activityName,
          this.queryForm.consumerName
        )
        .then(res => {
          this.pageCount = res;
          this.pageIndex = 1;
          this.getPage(1);
        });
    },
    getPage(pageIndex) {
      api
        .getPage(
          pageIndex,
          this.queryForm.className,
          this.queryForm.activityName,
          this.queryForm.consumerName
        )
        .then(res => {
          this.tableData = res;
        });
    },
    create() {
      this.entityForm = {
        id: -1,
        consumerId: null,
        activityId: null,
        dailyScore: null,
        examScore: null,
        score: null
      };
      this.editing = true;
    },
    edit(id) {
      api.get(id).then(res => {
        this.entityForm = res;
        this.editing = true;
      });
    },
    save() {
      if (this.entityForm.id === -1) {
        api.create(this.entityForm).then(() => {
          this.finishSave();
        });
      } else {
        api.update(this.entityForm).then(() => {
          this.finishSave();
        });
      }
    },
    finishSave() {
      this.$message.success("成功");
      this.getPage(this.pageIndex);
      this.editing = false;
    },
    deleteItem(id) {
      api.deleteItem(id).then(() => {
        this.$message.success("删除成功");
        this.getPage(this.pageIndex);
      });
    },
    getActivitysAndConsumers() {
      ActivityApi.listName().then(res => {
        this.activitys = res;
      });
      ConsumerApi.listName().then(res => {
        this.consumers = res;
      });
    },
    getSelectAndGradeStatus() {
      OptionApi.getAllowConsumerSelect().then(res => {
        this.optionForm.allowSelect = res;
      });
      OptionApi.getAllowTeacherGrade().then(res => {
        this.optionForm.allowGrade = res;
      });
    },
    updateAllowSelect(val) {
      OptionApi.setAllowConsumerSelect(val);
    },
    updateAllowGrade(val) {
      OptionApi.setAllowTeacherGrade(val);
    }
  },
  created() {
    this.query();
    this.getActivitysAndConsumers();
    this.getSelectAndGradeStatus();
  }
};
</script>

<style scoped></style>
